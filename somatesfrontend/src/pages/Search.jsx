...
// export default function Search({ navigate ,onLogout }) {
...
//             style={{display:"flex",alignItems:"center",gap:"0.5rem",padding:"0.625rem 0.875rem",borderRadius:"0.75rem",border:"none",cursor:"pointer",background:"transparent",fontSize:"0.875rem",color:T.onVariant,marginTop:"0.5rem"}} onMouseEnter={e=>{e.currentTarget.style.background="#fef2f2";e.currentTarget.style.color="#ef4444";}} onMouseLeave={e=>{e.currentTarget.style.background="transparent";e.currentTarget.style.color=T.onVariant;}}>
...
// ── FIXED: UserCard with privacy-aware follow / request / unfollow logic ──
function UserCard({ user, onMessage, onProfile }) {
  const [following, setFollowing] = useState(user.is_following || false);
  const [requested, setRequested] = useState(user.is_requested || false);
  const [loading, setLoading] = useState(false);
  ...
  const toggle = async () => {
    ...
    try {
      if (following) {
        ...
        await fetch(`${BASE_URL}/unfollow/${user.id}`, { method: "DELETE", credentials: "include" });
        setFollowing(false);
        setRequested(false);
      } else if (requested) {
        // Request already sent → cancel it
        await fetch(`${BASE_URL}/follow_request/${user.id}`, { method: "DELETE", credentials: "include" });
        setRequested(false);
      } else if (user.is_private) {
        // Private account → send a follow request, do NOT follow directly
        await fetch(`${BASE_URL}/follow_request/${user.id}`, { method: "POST", credentials: "include" });
        setRequested(true);
      } else {
        // Public account → direct follow as before
        await fetch(`${BASE_URL}/following/${user.id}`, { method: "POST", credentials: "include" });
        setFollowing(true);
      }
    } catch {}
    finally { setLoading(false); }
  ...
  // Derive button label + style
  const btnLabel = loading ? "…" : following ? "Following" : requested ? "Requested" : "Follow";
  const btnColor = following ? "white" : requested ? T.onVariant : T.primary;
  const btnBg    = following ? T.primary : requested ? T.container : "rgba(110,59,216,0.09)";
  ...
  return (
    <div className="fu" style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"1rem 1.25rem",borderRadius:"1.25rem",background:T.glass,backdropFilter:"blur(40px)",border:"1px solid rgba(255,255,255,0.6)"}}>
      ...
      <div style={{display:"flex",gap:"0.5rem",flexShrink:0}}>
        ...
        <button onClick={toggle} disabled={loading} style={{padding:"0.4rem 1rem",borderRadius:"9999px",border:"none",cursor:"pointer",fontSize:"0.8rem",fontWeight:700,transition:"all 0.2s",color:btnColor,background:btnBg,opacity:loading?0.5:1}}>
          {btnLabel}
        </button>
      </div>
    </div>
