...
export default function People({onLogout}) {
  ...
  // ── FIXED: privacy-aware follow / request / unfollow / cancel-request logic ──
  const toggleFollow = async (userId, isFollowing, isPrivate, isRequested, e) => {
    e.stopPropagation();
    try {
      if (isFollowing) {
        // Already following → unfollow
        await fetch(`${BASE_URL}/unfollow/${userId}`, { method: "DELETE", credentials: "include" });
        setUsers(prev => prev.map(u => u.id === userId ? { ...u, is_following: false, is_requested: false } : u));
      } else if (isRequested) {
        // Request already sent → cancel it
        await fetch(`${BASE_URL}/follow_request/${userId}`, { method: "DELETE", credentials: "include" });
        setUsers(prev => prev.map(u => u.id === userId ? { ...u, is_requested: false } : u));
      } else if (isPrivate) {
        // Private account → send a follow request, do NOT follow directly
        await fetch(`${BASE_URL}/follow_request/${userId}`, { method: "POST", credentials: "include" });
        setUsers(prev => prev.map(u => u.id === userId ? { ...u, is_requested: true } : u));
      } else {
  ...
  // ── Derive button label + style based on follow state ──
  const getFollowButtonProps = (u) => {
    if (u.is_following) {
      return { label: "Following", color: "white", background: T.primary };
    }
    if (u.is_requested) {
      return { label: "Requested", color: T.onVariant, background: T.container };
    }
    return { label: "Follow", color: T.primary, background: "rgba(110,59,216,0.09)" };
  };
        ...
        <main className="main-content" style={{ paddingTop:"5.5rem", paddingBottom:"6rem" }}>
          <div style={{ maxWidth:"40rem", margin:"0 auto", display:"flex", flexDirection:"column", gap:"1.25rem" }}>
            ...
            {loading ? (
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:"0.625rem" }}>
                {filtered.map((u, i) => {
                  const btnProps = getFollowButtonProps(u);
                  return (
                    <div
                      key={u.id}
                      className="fu user-row"
                    ...
                    >
                      ...
                      >
                        ...
                        <div style={{ display:"flex", gap:"0.5rem", flexShrink:0 }}>
                          ...
                          <button
                            onClick={(e) => toggleFollow(u.id, u.is_following, u.is_private, u.is_requested, e)}
                            style={{ padding:"0.45rem 1rem", borderRadius:"9999px", border:"none", cursor:"pointer", fontSize:"0.78rem", fontWeight:700, transition:"all 0.2s", color: btnProps.color, background: btnProps.background }}
                          >
                            {btnProps.label}
                          </button>
                        </div>
