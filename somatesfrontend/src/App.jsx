...
// ── Protected: logged-out users → /login ──────────────────────────────────
function Protected({ children, authStatus }) {
  if (authStatus === "loading") return <LoadingScreen />;
  if (authStatus === "unauth")  return <Navigate to="/login" replace />;
  return children;
...
// ── Page wrappers ──────────────────────────────────────────────────────────
// onLogout is passed into every page so sidebar/nav logout buttons work
function FeedPage({ onLogout }) {
  const nav = useNavigate();
  return <UserFeed navigate={makeNavigate(nav)} onLogout={onLogout} />;
}
function MessagesPage({ onLogout }) {
  const nav = useNavigate();
  return <Messages navigate={makeNavigate(nav)} onLogout={onLogout} />;
}
function ProfilePage({ onLogout }) {
  const nav = useNavigate();
  return <Profile navigate={makeNavigate(nav)} onLogout={onLogout} />;
}
function SearchPage({ onLogout }) {
  const nav = useNavigate();
  return <Search navigate={makeNavigate(nav)} onLogout={onLogout} />;
}
function PeoplePage({ onLogout }) {
  const nav = useNavigate();
  return <People navigate={makeNavigate(nav)} onLogout={onLogout} />;
}
  ...
  return (
    ...
    <BrowserRouter key={logoutKey}>
      <Routes>
        ...
        <Route path="/people" element={
          <Protected authStatus={authStatus}>
            <PeoplePage onLogout={handleLogout} />
          </Protected>
        }/>
